...
export default function BlogList({imgurl,title,date,author}) {
    return (
        <div className="flex flex-col border border-gray-300 bg-white  shadow-md rounded-3xl mt-10 mb-10 w-[1000px] ">
            <img
                src={imgurl}
                className="w-full h-[500px] object-cover rounded-xl"/>
                 <div className="flex gap-8 text-gray-400 text-lg mt-10 px-10 ">
                    <p>{date}</p>
                    <p>👤 {author}</p>
                </div>
                <div className="flex flex-col text-2xl w-full h-[150px] text-md font-light text-gray-500  mt-5 px-10">
